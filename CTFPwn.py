...
if len(sys.argv) != 5:
    ...
    print("Example:")
    print("sudo python3 ~/tools/PythonScripts/CTFPwn.py 10.10.11.239 ~/current/working/directory ~/directoy/wordlist 0 (A faster scan for CTF's)")
    print("sudo python3 ~/tools/PythonScripts/CTFPwn.py 10.10.11.239 ~/current/working/directory ~/direcoty/wordlist 1 (A slower scan for real-world stealth)")
    sys.exit(1)
...
while True:
    Tor = input("Tor Y/N: ")
    if Tor.upper() == 'Y':
        Tor = '--proxy socks4://127.0.0.1:9050'
...
RUSTcommand = f"sudo rustscan  -t 2000 -b 2000 --ulimit 5000 -r 0-65535 -a {IP}"
...
NMAPcommand = f"sudo nmap -Pn -sV -sU -p- {rate} {Tspeed} -vvv {IP} {Tor} -oA Nmap/ports"
...
NMAP1command = "sudo nmap -A -p"+",".join(map(str, open_ports_list))+f" {rate} {Tspeed} -vvv {IP} {Tor} -oA Nmap/mapped"
